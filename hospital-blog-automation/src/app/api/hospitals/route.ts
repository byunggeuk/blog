...
import { getHospitals, addHospital, updateHospital, deleteHospital } from '@/lib/google-sheets';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { hospital_name, blog_url, reference_folder_id, output_folder_id, prompt_name, system_prompt } = body;

    if (!hospital_name) {
      return NextResponse.json({ error: '병원 이름은 필수입니다.' }, { status: 400 });
    }

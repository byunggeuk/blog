      ...
      return NextResponse.json(
        {
          ...
          hint:
            openData.error === "user_not_found"
              ? "슬랙 멤버 ID가 잘못되었거나 Bot이 해당 워크스페이스에 없습니다."
              : openData.error === "invalid_auth"
                ? "SLACK_BOT_TOKEN이 유효하지 않습니다."
                : openData.error === "missing_scope"
                  ? "Bot에 im:write 권한이 필요합니다."
                  : "",
        },
        { status: 400 },
      );
      ...
      return NextResponse.json(
        {
          ...
          hint:
            postData.error === "channel_not_found"
              ? "채널을 찾을 수 없습니다."
              : postData.error === "not_in_channel"
                ? "Bot이 채널에 없습니다."
                : postData.error === "missing_scope"
                  ? "Bot에 chat:write 권한이 필요합니다."
                  : "",
        },
        { status: 400 },
      );
